...
G.updateMovement = function () {
    ...
    if (G.keys.ArrowLeft || G.keys.a) { p.x -= p.speed; p.facingRight = false; isMoving = true; }
    if (G.keys.ArrowRight || G.keys.d) { p.x += p.speed; p.facingRight = true; isMoving = true; }
...
G.updateCamera = function () {
    ...
    if (p.invincibleTimer > 0) { p.invincibleTimer--; } else { p.isInvincible = false; }
